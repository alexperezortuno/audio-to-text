...
d = {'clientip': '192.168.0.1', 'user': 'sdloggs'}
...
#                               mode='x', samplerate=SAMPLE_RATE,
#                               channels=CHANNELS, subtype=None) as file:
#                 with sounddevice.InputStream(samplerate=SAMPLE_RATE, device=self.mic_id,
...
def record_audio():
    print("recording...")
    print(FOLDER_PATH)
    print(os.path.join(FOLDER_PATH, 'audio_files'))
    # record = sounddevice.rec(int(second * SAMPLE_RATE), samplerate=SAMPLE_RATE, channels=CHANNELS)
    # sounddevice.wait()
    # write("output.wav", SAMPLE_RATE, record)
